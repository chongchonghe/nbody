...
parameters:
\begin{itemize}
	\item The \textit{close-encounter parameter} $ \alpha $ which confirms that they are 
	close enough to each other so that we start to do further test:
	\begin{equation}\label{eq:r12}
	d_{\rm close} = | \v{r}_1 - \v{r}_2 | < \alpha \cdot d_{\rm virial}.
	\end{equation}
...
step} = \sigma \Delta t $, the close-encounter parameter $ \alpha $, and the escape 
parameter $ \beta $.
